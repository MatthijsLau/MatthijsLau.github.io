...
\definecolor{mydarkblue}{HTML}{1f2b41}
\definecolor{myblue}{HTML}{304263}
\definecolor{mylightblue}{HTML}{496497}

%\usepackage[hidelinks]{hyperref} % For clickable links in the PDF
\usepackage[colorlinks = true, urlcolor = myblue]{hyperref} % For clickable links in the PDF
...
\makeatletter
\let\old@rule\@rule
\def\@rule[#1]#2#3{\textcolor{mydarkblue}{\old@rule[#1]{#2}{#3}}}
\makeatother
...
\newenvironment{sectie}[1]{
	...
	\begin{tcolorbox}[
		...
		width = \textwidth,
		title = {\Large{#1}},
		after title = {\par\vspace*{-2.8ex}{\color{mydarkblue}\vspace{5pt}\rule{\textwidth}{2pt}}\vspace*{-1.4ex}}
		]
	\vspace{1ex}
}{
	\end{tcolorbox}
...
\newenvironment{ondersectie}[4][]{
	\ifthenelse{
		...
		\ifthenelse{
				...
				\par\vspace*{-4.2ex}{\color{mylightblue}\vspace{5pt}\rule{\textwidth}{.7pt}}\vspace{-2.8ex}
			}{
			}
		}{
		\ifthenelse{
...
\newcommand{\head}[1]{\vspace*{1.5ex}{\Large\color{myblue}#1}\par\vspace*{-2.8ex}{\color{myblue}\vspace{5pt}\rule{\textwidth}{2pt}}\par}
...
\begin{minipage}{0.4\textwidth}
	\null\hfill\includegraphics[height=0.2\textheight]{pic5.jpg}\hfill\null
\end{minipage}
...
\begin{listsectie}{Skills}
	\comp Languages: Dutch (Native), English (C2)
